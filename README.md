...
FrameFlow 把文本、音频或视频输入转成持久化异步任务，生成语义片段，为每个片段给出至少 3 个带分项得分和中文理由的素材候选，并允许人工编辑字幕、拖动排序、搜索替换素材。系统还能把最终选择组织成可视化时间线，并异步生成带匹配素材的 MP4 组合预览。页面刷新后，任务进度、最终选择、预览结果、AI/规则运行记录和审计记录仍然存在。
...
后端测试覆盖分段、关键词、自动标签、混合排序、语义回退、任务恢复、并发幂等、限流、时间线和预览渲染契约。`evaluation/evaluate.py` 在 34 条字幕（24 easy + 10 hard）上对比关键词基线 / 字符 TF-IDF / 混合排序，并在启用 embedding 时追加“混合排序(向量)”行。2026-07-14 本地结果为向量混排 Hit@3 `0.9412`、nDCG@3 `0.8288`。更完整的验收范围见 [测试计划](docs/TEST_PLAN.md) 和 [评分矩阵](docs/SCORING_MATRIX.md)。
...
- 当前版本没有应用内用户登录和租户隔离；已有进程内读写限流与部署层 Basic Auth，但它们不等同于多租户授权或分布式配额。
...
5. 注入 `ai_degrade`，展示规则接管与运行记录中的降级标识。
6. 注入 `job_fail`，展示失败原因、可重试判断与成功恢复。
7. 打开 AI 运行记录、审计记录、健康检查和公开 Git 提交历史。
