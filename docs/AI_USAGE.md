...
本项目的需求拆解、架构备选方案、部分代码初稿、UI 初稿、测试用例和交付文档使用了 AI 编程助手 Codex 辅助生成。这一使用范围应在版本记录和技术评审中如实披露。

AI 辅助不代表：

- 代码、命令和文档无需人工复核。
- 开发者无需理解架构决策、数据模型或故障处理。
- 自动生成的测试名称可以代替真实执行记录。
- 可以把故障注入、样例数据或规则输出宣称为外部大模型的真实结果。

发布前，维护者应至少亲自完成：复跑安装/测试命令、在目标环境走完主流程与故障流程、理解关键架构决策与运行边界，并修改任何与最终实现不一致的文档。
...
1. 演示或验收环境不应因 API Key、余额、网络或 Provider 限流无法走完核心闭环。
2. 新字幕的输出必须可重现，方便单元测试和故障定位。
3. 混合公式的每个分量都可向技术审阅者解释，且可保存命中词证据。
