...
- 最低 2 核 CPU、2 GB 内存、20 GB 磁盘；构建本地 ASR 镜像建议 4 GB 以上内存。
...
脚本会创建 `deploy/.env`、写入域名与同源 CORS、校验 Compose、构建镜像、启动服务并等待就绪。首次构建通常需要数分钟。Caddy 在 DNS 与端口正确时自动申请和续期证书。
...
| `BASIC_AUTH_USER` | 可选 Caddy 整站鉴权用户名 | `frameflow` |
| `BASIC_AUTH_HASH` | 可选 Caddy bcrypt 密码哈希；建议在 `.env` 中用单引号包裹 | 空，默认不启用 |
...
### 可选：为公开 Demo 启用整站 Basic Auth

默认部署不启用密码，避免评审打开链接时被意外锁在登录框外。准备公开演示时，可用以下步骤保护包括删除、上传和故障注入在内的全部页面/API：

```bash
# 1. 生成 bcrypt 哈希；交互输入密码，避免密码进入 shell history
read -rsp "Demo password: " PASSWORD; echo
HASH="$(printf '%s' "$PASSWORD" | docker run --rm -i caddy:2.10-alpine caddy hash-password)"
unset PASSWORD

# 2. 编辑 deploy/.env，设置用户名，并把哈希用单引号包裹，防止 $ 被 Compose 展开
BASIC_AUTH_USER=frameflow
BASIC_AUTH_HASH='$2a$14$请替换为上一步完整输出'

# 3. 启用片段并重建 Caddy
cp deploy/caddy/basic-auth.caddy.example deploy/caddy/10-basic-auth.caddy
docker compose --env-file deploy/.env up -d --force-recreate caddy
```

先在新的终端或无痕窗口确认账号可登录，再保留当前管理会话。关闭鉴权：

```bash
rm deploy/caddy/10-basic-auth.caddy
docker compose --env-file deploy/.env up -d --force-recreate caddy
```

`10-basic-auth.caddy` 被本地 `.gitignore` 排除，不会把站点保护策略误提交到公开仓库。Basic Auth 必须配合 HTTPS 使用；本方案由 Caddy 自动提供 HTTPS。
...
归档默认写入 `backups/frameflow-UTC时间.tar.gz`。请同步到服务器之外；同机单份备份不能应对磁盘故障。
...
恢复脚本会拒绝包含绝对路径或 `..` 路径穿越的归档，随后重建服务并等待就绪。恢复前仍建议额外保存当前数据。
...
- 应用以非 root 用户运行，并启用 `no-new-privileges`。
- 当前演示版没有登录、租户隔离、速率限制和恶意内容扫描，不建议匿名长期暴露公网。
- 删除、上传和演示故障注入接口当前没有独立鉴权。公开面试 Demo 应优先通过云防火墙白名单、VPN/Tailscale 或 Caddy Basic Auth 限制访问；不要把它当作匿名公共 SaaS。
