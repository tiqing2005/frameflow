...
脚本会创建 `deploy/.env`、写入域名与同源 CORS，并依次要求设置应用管理员密码和 Caddy Demo 入口密码。应用密码生成 PBKDF2-SHA256 哈希，入口密码生成 bcrypt 哈希；两者都不会以明文写入配置。公网部署会关闭浏览器远程首次认领，只接受预置管理员凭据。随后脚本验证 Compose、应用 ready、Caddy 容器健康与配置，以及公网 HTTPS/Basic Auth；任一必需检查失败都不会报告部署成功。
