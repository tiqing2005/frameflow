...
当前 Demo 按“单实例、有界 Worker 进程池、中小素材库”设计，默认允许 2 个任务同时执行。它不声称支持水平扩容、大文件长视频处理或多租户生产负载。
...
| 有界 Worker 池（默认 2） | 同机并行执行并以原子领取、租约栅栏控制 SQLite 写竞争 | PostgreSQL + 专用队列 + 横向 Worker |
