...
本文描述 FrameFlow 1.0 当前使用的 SQLite/SQLAlchemy 持久化模型。它服务于五个核心不变量：
...
运行控制实体 `IDEMPOTENCY_RECORD`、`FAULT_CONTROL` 和 `WORKER_HEARTBEAT` 不参与业务对象展示，但为幂等、故障演练和 readiness 提供持久化依据。`AUTH_IDENTITY` 与 `AUTH_SESSION` 通过规范化 username 形成逻辑关系，当前 schema 没有声明外键；删除或替换身份时必须由认证服务显式清理会话。
...
当前版本不单独建立 `match_runs` 表；一次混合匹配的策略版本和结果摘要统一保存在 AIRun，具体候选保存在 Recommendation。
...
Worker 在领取相关任务时应以事务方式读取并恢复 `none`，确保故障只被一个任务消费。该表只服务于受控可靠性演练，不是通用的生产 Feature Flag 系统。
...
本地首次初始化只允许从服务所在机器的回环地址创建该 singleton 身份。托管部署也可完全使用环境变量提供的密码哈希；环境身份不会复制进此表。当前采用单管理员部署模式，不包含注册、RBAC、租户或资源归属关系。
...
登录成功只把随机原始 session token 写入 HttpOnly Cookie，数据库保存 `token_hash`。退出会删除当前会话；查询会话时同时检查到期时间。CSRF token 可由会话接口返回给同源前端，但不得替代 HttpOnly 会话 Cookie。该设计覆盖单实例部署的应用会话，不声称具备集中式 SSO 或分布式会话能力。
...
- FrameFlow 1.0 启动时由 SQLAlchemy metadata 创建缺失的表，再幂等 seed 本地素材。
- 当前方式适合单实例快速初始化，但不支持可审核的破坏性 schema 升级。进入长期迭代后应引入 Alembic 版本迁移。
